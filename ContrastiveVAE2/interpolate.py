...
def interpolate():
    ...
    with torch.no_grad():
        for _ in range(500):
            ...
            trajectory = {
                "source_index": i,
                "destination_index": j,
                "source_name": train_names[i][0],
                "destination_name": train_names[j][0],
                "alphas": [],
                "generated": [],
            }

            x1, length1, label1 = train_dataset[i]
            x2, length2, label2 = train_dataset[j]

            x1 = x1.unsqueeze(0).to(device)
            x2 = x2.unsqueeze(0).to(device)

            length1 = torch.tensor([length1], device=device)
            length2 = torch.tensor([length2], device=device)
    ...
    # Novelty
    print(f"Novelty wrt training data: {compute_novelty(generated, train_dataset):.2%}")
