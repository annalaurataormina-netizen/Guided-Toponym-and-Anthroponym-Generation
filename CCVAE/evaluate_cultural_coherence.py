...
def evaluate_cultural_coherence(generated_per_language, language_to_id, device, vocab, names_normalised):
    ...
    with torch.no_grad():
        ...
        for language, label in sorted(language_to_id.items(), key=lambda x: x[1]):
            ...
            data = [[name, label] for name in generated_names]
            ...
            top2_accuracies.append(top2_generation_accuracy)
            top3_accuracies.append(top3_generation_accuracy)
            top4_accuracies.append(top4_generation_accuracy)
            top5_accuracies.append(top5_generation_accuracy)
            top6_accuracies.append(top6_generation_accuracy)
            top7_accuracies.append(top7_generation_accuracy)
            top8_accuracies.append(top8_generation_accuracy)
            top9_accuracies.append(top9_generation_accuracy)
            top10_accuracies.append(top10_generation_accuracy)

    avg_generation_accuracy = sum(accuracies) / len(accuracies)
    avg_top2_generation_accuracy = sum(top2_accuracies) / len(top2_accuracies)
    avg_top3_generation_accuracy = sum(top3_accuracies) / len(top3_accuracies)
    avg_top4_generation_accuracy = sum(top4_accuracies) / len(top4_accuracies)
    avg_top5_generation_accuracy = sum(top5_accuracies) / len(top5_accuracies)
    avg_top6_generation_accuracy = sum(top6_accuracies) / len(top6_accuracies)
    avg_top7_generation_accuracy = sum(top7_accuracies) / len(top7_accuracies)
    avg_top8_generation_accuracy = sum(top8_accuracies) / len(top8_accuracies)
    avg_top9_generation_accuracy = sum(top9_accuracies) / len(top9_accuracies)
    avg_top10_generation_accuracy = sum(top10_accuracies) / len(top10_accuracies)

    return ({
        "Average generation accuracy": avg_generation_accuracy,
        "Average top-2 generation accuracy": avg_top2_generation_accuracy,
        "Average top-3 generation accuracy": avg_top3_generation_accuracy,
        "Average top-4 generation accuracy": avg_top4_generation_accuracy,
        "Average top-5 generation accuracy": avg_top5_generation_accuracy,
        "Average top-6 generation accuracy": avg_top6_generation_accuracy,
        "Average top-7 generation accuracy": avg_top7_generation_accuracy,
        "Average top-8 generation accuracy": avg_top8_generation_accuracy,
        "Average top-9 generation accuracy": avg_top9_generation_accuracy,
        "Average top-10 generation accuracy": avg_top10_generation_accuracy,
    })
