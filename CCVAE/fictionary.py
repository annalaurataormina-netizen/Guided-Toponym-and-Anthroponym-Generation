...
def fictional_culture():
    ...
    #
    # A higher threshold is used here to ensure that each
    # parent culture has sufficient data to support reliable
    ...
    # samples. This threshold is intentionally different from
    # the >=10000 threshold used for selecting parent cultures.
    # ---------------------------------------------------------
    ...
    with torch.no_grad():
        ...
        for pair_number, (culture_a, culture_b) in enumerate(
            ...
            # -------------------------------------------------
            # N-GRAM PARENT-CULTURE RANKING
            # -------------------------------------------------

            # Perform ranking independently for each n-gram
            # order. For each generated name, cultures are
            # ranked by n-gram log-probability.
            #
            # A name counts as a parent match if either of its
            # two parent cultures appears within the top-k.
            # -------------------------------------------------
            ...
            for name in generated:
                ...
                # Rank all classifier cultures by their logits.
                ranked_predictions = torch.argsort(
                    logits[0],
                    descending=True
                ).tolist()

                ranked_cultures = [
                    classifier_id_to_language[prediction]
            ...
            # -------------------------------------------------
            # CULTURE CLASSIFIER PARENT-CULTURE RANKING
            # -------------------------------------------------

            classifier_rank_counts = {
                k: 0
                for k in range(1, 11)
            }

            for ranked_cultures in classifier_rankings:

                for k in classifier_rank_counts:
                    ...
                    if parent_cultures & top_k:
                        classifier_rank_counts[k] += 1

            print()
            print("CULTURE CLASSIFIER PARENT RANKING")

            for k in range(1, 11):

                rate = (
                    classifier_rank_counts[k]
                    / n_generated
                )
            ...
            # -------------------------------------------------
            # CLASSIFIER TOP-1 PREDICTION COUNTS
            # -------------------------------------------------
            ...
            print()
            print("CULTURE CLASSIFIER TOP-1 PREDICTIONS")

            print(
                f"Parent cultures: "
                f"{parent_predictions / n_generated:.2%}"
            )

            print(
                f"Culture A: "
                f"{prediction_counts[culture_a] / n_generated:.2%}"
            )

            print(
                f"Culture B: "
                f"{prediction_counts[culture_b] / n_generated:.2%}"
            )
    ...
    print()
    print("CULTURE CLASSIFIER PARENT-CULTURE RANKING")
    print()

    for k in range(1, 11):

        values = [
            result["classifier_ranking"][k]
            for result in pair_results
        ]
    ...
    # ---------------------------------------------------------
    # CULTURE CLASSIFIER TOP-1 PREDICTIONS
    # ---------------------------------------------------------
    ...
    print(
        "N-gram ranking measures whether either parent culture "
        "appears among the highest-scoring cultures for each "
        "generated name."
    )

    print(
        "The culture classifier independently measures whether "
        "either parent culture appears among the classifier's "
        "top-k predictions."
    )
